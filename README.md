...
| Primary | Overrides / fallback | Description |
|---------|----------------------|-------------|
| `config/workflows/spec-gate/spec-gate.config` → **`model`** | Repo variable **`GATE_MODEL`**, then legacy **`GATE_CODEX_MODEL`**, then **`o4-mini`** | Codex model for spec gate analysis |
