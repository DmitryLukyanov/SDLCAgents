...
import { runPipeline } from '../runners/pipeline.js';
import type { PipelineStep } from '../runners/runner-types.js';
...
async function main(): Promise<void> {
  ...
  if (runner === 'pipeline') {
    ...
    await runPipeline(issueKey, steps);
    return;
  }

  throw new Error(
    `Unknown params.runner "${runner}" in ${configFile}. Supported: dummy_ticket, pipeline`,
  );
}
