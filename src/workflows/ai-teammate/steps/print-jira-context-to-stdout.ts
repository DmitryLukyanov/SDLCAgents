/**
 * Ticket processor: load one Jira issue, enforce required status, optional related-ticket context, print.
 */
import { getRequiredIssueStatus, statusAllowsRead } from '../../../lib/jira-status.js';
...
export async function runPrintJiraContextToStdout(deps: AiTeammateDeps): Promise<void> {
  ...
  const issue = await deps.getIssue(issueKey, fields);
  ...
  console.log('--- Jira ticket (ticket processor) ---');
  ...
  if (depth >= 1) {
    ...
    const related = await deps.fetchRelatedIssueSummaries(issueKey, depth);
    if (related.length === 0) {
