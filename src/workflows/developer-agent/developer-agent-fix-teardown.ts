 ...
 *
 * Runs AFTER the dedicated Codex job (workspace changes already committed + pushed) in the fix workflow.
 *
 * Responsibilities:
 *  1. Touch speckit-state.json (lastFixedAt) → triggers spec gate re-validation
 *  2. Commit state + push
 *  3. Write GitHub Actions job summary (Codex workspace details are on the Codex job)
 *  4. Post PR comment
 *
...
async function main(): Promise<void> {
  ...
  // ── 1. Touch speckit-state.json → triggers spec gate re-validation ─
  ...
  // ── 3. Write GitHub Actions job summary ──────────────────────────
